...
if(response.statusCode==200&&response.data["message"]=="تم الحصول على بيانات جميع العقارات بنجاح"){
  ...
  if (e is DioException) {
   ...
   @override
  Future<Either<failure, String>> deleteaqar({required String token, required int aqarnumber})async {
   try{
Response response=await Deletedata.deletedata(path: "/state/delete/${aqarnumber}",token: token);
if(response.statusCode==200&&response.data["message"]=="تم حذف بيانات العقار  بنجاح")return right("تم حذف البيانات بنجاح");
else return left(requestfailure(error_message: response.data["message"]));
   }catch(e){
     if (e is DioException) {
        return left(requestfailure.fromdioexception(e));
